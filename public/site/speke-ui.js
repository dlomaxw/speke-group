...
(function () {
  ...
  function dismissLoader() {
    ...
    /* The server renders the loader, so hide it rather than pulling a
       node out from under React. */
    window.setTimeout(function () { el.style.display = 'none'; }, 1300);
  }
  ...
  function initEnquiryForm() {
    ...
    for (var f = 0; f < forms.length; f++) {
      (function (form) {
        if (form.dataset.sgWired === '1') return;
  ...
  /* ---------- 12. Card click reveal ---------------------- */
  /* A linked card plays a short lift-and-sheen before the visitor leaves.
     Property sites open in a new tab straight away (delaying that would
     trip popup blockers); same-tab links wait for the animation. */
  var cardRevealWired = false;
  function initCardReveal() {
    if (cardRevealWired) return;
    cardRevealWired = true;
    document.addEventListener('click', function (ev) {
      if (ev.defaultPrevented || ev.button !== 0) return;
      var card = ev.target.closest && ev.target.closest('a.card, a.tile');
      if (!card) return;
      var href = card.getAttribute('href') || '';
      if (!href || href === '#') return;

      card.classList.remove('sg-opening');
      void card.offsetWidth;
      card.classList.add('sg-opening');
      window.setTimeout(function () { card.classList.remove('sg-opening'); }, 900);

      var newTab = card.target === '_blank' || ev.metaKey || ev.ctrlKey || ev.shiftKey || ev.altKey;
      if (newTab || reduced || href.charAt(0) === '#') return;
      ev.preventDefault();
      window.setTimeout(function () { window.location.href = card.href; }, 420);
    });
  }
